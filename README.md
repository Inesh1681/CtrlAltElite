...
48-district city with realistic terrain receives a storm; water accumulates, drains, and
...
src/
  simulation/          ← pure, deterministic, no React
    ...
    scenarios.ts       NORMAL RAIN / HEAVY MONSOON / EXTREME STORM / custom / live-forecast profiles
    engine.test.ts     16 vitest cases (determinism, conservation, downhill flow, thresholds…)
...
water'      = water + rainInput + inflow − outflow − drained − seaOutflow
```
...
- Synthetic city and terrain; no real DEM or GIS layers. Open-Meteo provides real rainfall only.
